...
import com.mhd.grit.core.presentation.settings.BackupState
...
class SettingsViewModel(
    ...
    // تم حذف BillingHandler نهائياً من هنا
) : ViewModel() {
    ...
    fun onAction(action: SettingsAction) = viewModelScope.launch {
        when (action) {
            ...
            SettingsAction.OnExport -> {
                _state.update { it.copy(backupState = it.backupState.copy(exportState = ExportState.EXPORTING)) }
                exportRepo.exportToJson()
                _state.update { it.copy(backupState = it.backupState.copy(exportState = ExportState.EXPORTED)) }
            }

            is SettingsAction.OnRestore -> {
                _state.update { it.copy(backupState = it.backupState.copy(restoreState = RestoreState.RESTORING)) }
            ...
            is SettingsAction.OnCheckBiometric -> {
                _state.update { it.copy(biometricAvailable = Utils.authenticationAvailable(action.context)) }
            }
            ...
            SettingsAction.OnPaywallShow -> {
                // منع ظهور شاشة الدفع نهائياً
                _state.update { it.copy(showPaywall = false) }
            }
    ...
    // الدالة المسؤولة عن حالة الإعلانات
    private suspend fun checkSubscription() {
        // قيمة false تعني أن المستخدم "عادي" وليس "Plus"، وهذا ما يفتح طريق الإعلانات
        val isSubscribed = false

        _state.update {
            it.copy(isUserSubscribed = isSubscribed)
        }
    ...
    private fun observeJob() = viewModelScope.launch {
        observeJob?.cancel()
        observeJob = launch {
            datastore.getTaskReorderPref().onEach { pref ->
                _state.update { it.copy(reorderTasks = pref) }
                stateLayer.tasksState.update { it.copy(reorderTasks = pref) }
            }.launchIn(this)

            datastore.getNotificationsFlow().onEach { pref ->
                _state.update { it.copy(pauseNotifications = pref) }
            }.launchIn(this)

            datastore.getAppThemeFlow().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(appTheme = flow)) }
            }.launchIn(this)

            datastore.getFontPrefFlow().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(font = flow)) }
            }.launchIn(this)

            datastore.getSeedColorFlow().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(seedColor = Color(flow))) }
            }.launchIn(this)

            datastore.getAmoledPref().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(isAmoled = flow)) }
            }.launchIn(this)

            datastore.getMaterialYouFlow().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(isMaterialYou = flow)) }
            }.launchIn(this)

            datastore.getPaletteStyle().onEach { flow ->
                _state.update { it.copy(theme = it.theme.copy(paletteStyle = flow)) }
            }.launchIn(this)

            datastore.getIs24Hr().onEach { flow ->
                _state.update { it.copy(is24Hr = flow) }
                stateLayer.tasksState.update { it.copy(is24Hour = flow) }
            }.launchIn(this)

            datastore.getStartingPagePref().onEach { flow ->
                _state.update { it.copy(startingPage = flow) }
            }.launchIn(this)

            datastore.getStartOfTheWeekPref().onEach { flow ->
                _state.update { it.copy(startOfTheWeek = flow) }
            }.launchIn(this)

            datastore.getBiometricLockPref().onEach { flow ->
                _state.update { it.copy(biometric = flow) }
            }.launchIn(this)
        }
    }
}